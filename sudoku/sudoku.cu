...
__device__ void obtain_constraints(size_t index, uint16_t* constraints, board_t board);

// This function keeps updating cells until no more changes are possible
__global__ void update_cells(board_t* boards, size_t num_boards) {
  // we don't need more blocks than num_boards
  if (blockIdx.x >= num_boards) return;

  while (true) {
    // store a copy of the cell
    uint16_t new_cell = boards[blockIdx.x].cells[threadIdx.x];

    // first update all constraints
    uint16_t constraints[BOARD_DIM * 3] = {0};
    obtain_constraints(threadIdx.x, constraints, boards[blockIdx.x]);
    // apply the constraints
    for (size_t i = 0; i < BOARD_DIM * 3; i++) {
      if (cell_to_digit(constraints[i]) > 0) {
        new_cell = new_cell & ~(1 << cell_to_digit(constraints[i]));
      } else
        continue;
    }

    // check whether we have updated anything
    int predicate = 0;
    if (new_cell != boards[blockIdx.x].cells[threadIdx.x]) predicate = 1;

    int thread_update_cnt = __syncthreads_count(predicate);

    // if no update, return; if there are, update each cell
    if (thread_update_cnt != 0) {
      boards[blockIdx.x].cells[threadIdx.x] = new_cell;
    } else {
      return;
    }
  }
...
void solve_boards(board_t* boards, size_t num_boards) {
  // create a copy of boards for gpu
  board_t* gpu_boards;
  // malloc memory for gpu_boards
  ...
  // run kernel
  update_cells<<<BATCH_SIZE, BOARD_DIM * BOARD_DIM>>>(gpu_boards, num_boards);
...
__device__ void obtain_constraints(size_t index, uint16_t* constraints, board_t board) {
  int quotient = index / BOARD_DIM;
  int remainder = index % BOARD_DIM;
  // row
  for (int i = 0; i < BOARD_DIM; i++) {
    if (i + BOARD_DIM * quotient != index) {
      constraints[i] = board.cells[i + BOARD_DIM * quotient];
    }
    // if it's the cell itself, we store to 0 (cell itself is not a constraint)
    else
      constraints[i] = 0;
  }
  ...
  // column
  for (int i = 0; i < BOARD_DIM; i++) {
    if ((remainder + BOARD_DIM * i) != index) {
      constraints[i + BOARD_DIM] = board.cells[remainder + BOARD_DIM * i];
    }
    // if it's the cell itself, we store to 0 (cell itself is not a constraint)
    else
      constraints[i + BOARD_DIM] = 0;
  }

  // group
  int group_row = quotient / GROUP_DIM;
  int group_col = remainder / GROUP_DIM;

  for (int i = 0; i < GROUP_DIM; i++) {
    for (int j = 0; j < GROUP_DIM; j++) {
      // calculate cell's index
      int cell_index = group_row * BOARD_DIM * 3 + group_col * GROUP_DIM + i * BOARD_DIM + j;
      if (cell_index != index) {
        constraints[i * 3 + j + 2 * BOARD_DIM] = board.cells[cell_index];
      }
      // if it's the cell itself, we store to 0 (cell itself is not a constraint)
      else
        constraints[i * 3 + j + 2 * BOARD_DIM] = 0;
    }
  }
}
